(define (domain dominioStarCraft_ej5)

(:requirements :strips :adl :fluents )

(:types
    Entidad Localizacion Recurso Investigacion TipoUnidad TipoEdificio TipoInvestigacion - object
...
(:predicates
    ...
    ;predicados de investigación
    (unidadInvestigada ?tu - TipoUnidad)
    (necesitaInvestigacion ?tu - TipoUnidad)
    (investigacionRealizada ?i - Investigacion)
    (investigacionEs ?i - Investigacion ?ti - TipoInvestigacion)
    (recursoParaInvestigar ?ti - TipoInvestigacion ?r - Recurso)
    ...
    :precondition (and
        ...
        (exists (?tu - TipoUnidad)
            (and
                ...
                ;comprobamos si necesita ser investigada
                (imply (and (necesitaInvestigacion ?tu))

                    ;si lo necesita comprobamos que se haya investigado
                    (and (unidadInvestigada ?tu))
                )
...
(:action Investigar
    ...
    :precondition (and
        ...
        ;la investigación no se ha hecho antes y estamos en una Bahía de Ingeniería para poder investigar
        (not (investigacionRealizada ?i))
        (edificioConstruido ?e)
        (edificioEs ?e BahiaIngenieria)

        ;se debe tener los recursos necesarios para la investigación
        (exists (?ti - TipoInvestigacion)
            (and
                (investigacionEs ?i ?ti)

                ;para todos los recursos, la investigación no necesita el recurso o lo necesita y se está extrayendo
                (forall (?r - Recurso)
                    (or
                        (not (recursoParaInvestigar ?ti ?r))
                        (and
                            (recursoParaInvestigar ?ti ?r)
                            (recursoObtenido ?r)
                        )
                    )
                )
    ...
    :effect (and
        ...
        (when (and (investigacionEs ?i InvSoldado))
            (and (unidadInvestigada Soldado))
        )
    )
)
