(define (domain dominioStarCraft_ej8)
...
(:functions
    ...
    (costeTiempoConstruirEdificio ?te - TipoEdificio)
...
(:action Navegar
    ...
    :effect (and 
        ...
        (when (unidadEs ?u VCE) 
            (and
                (increase (costeTiempo) (costeTiempoMover VCE))
            )
        )

        (when (unidadEs ?u Soldado) 
            (and
                (increase (costeTiempo) (costeTiempoMover Soldado))
            )
        )

        (when (unidadEs ?u Marine) 
            (and
                (increase (costeTiempo) (costeTiempoMover Marine))
            )
        )
    )
)
    ...
    :effect (and
        ...
        (when (and (edificioEs ?e Barracones)) 
            (and
                ...
                (increase (costeTiempo) (costeTiempoConstruirEdificio Barracones))
            )
        )
        ...
        (when (and (edificioEs ?e Extractor)) 
            (and
                ...
                (increase (costeTiempo) (costeTiempoConstruirEdificio Extractor))
            )
        )
...
(:action Reclutar
    ...
    :precondition (and
        ...
        ;comprobar si se quiere reclutar en el edificio correcto y se tienen los recursos necesarioscat 
